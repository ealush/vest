...
| PF04 | Compare creation and full-run behavior against base on the same machine. G1 pair budget (revised 2026-09-14, explicitly accepted): 0.80 hard floor with 0.90 target — warn-band (0.80–0.90) passes the gate only with the 5μs/edge absolute-overhead ceiling also passing; below-floor breaches fail. Rationale: creation is one-time per schema, relationship-free schemas take the metadata fast path, and observed overhead is ~1–2μs/edge (≈0.84–0.88 ratio, ~13μs per plain 2-field shape vs pre-feature base). Base-singles (A1/C12full/D13full) keep the >10% regression blocker with CV ≤ 0.20; noisy samples retry both head and baseline once, then fail inconclusive — never success. |
