...
| PF04 | Compare creation and full-run behavior against base on the same machine. G1 pair budget (revised 2026-09-14, explicitly accepted): 0.80 hard floor with 0.90 target — warn-band (0.80–0.90) passes the gate only with the 5μs/edge absolute-overhead ceiling also passing; below-floor breaches fail. Rationale: creation is one-time per schema; observed related/plain ratio ≈0.84–0.91 with ~1–2μs/edge. A1 creation-vs-base is bounded absolutely (500ms per 20k-shape batch, observed ~290ms), not relatively: the pre-feature base lacks the relationship machinery, so a relative A1 comparison measures feature existence (~11x), not regression. C12full/D13full keep the >10% relative blocker with CV ≤ 0.20; noisy samples retry both head and baseline once, then fail inconclusive — never success. |
