...
describe('isArray', () => {
  it('passes for empty arrays', () => {
    expect(isArray<any>().run([]).passes).toBe(true);
  });

  it('passes for arrays with elements', () => {
    expect(isArray<number>().run([1, 2]).passes).toBe(true);
    expect(isArray<string>().run(['a', 'b']).passes).toBe(true);
    expect(isArray<any>().run([null, undefined]).passes).toBe(true);
    expect(isArray<any>().run([{}, []]).passes).toBe(true);
  });

  it('fails for array-like objects', () => {
    const arrayLike: any = { 0: 'a', 1: 'b', length: 2 };
    expect(isArray<any>().run(arrayLike).passes).toBe(false);
  });
  ...
  it('fails for non-arrays', () => {
    ...
    values.forEach(value => {
      expect(isArray<any>().run(value).passes).toBe(false);
    });
  });
  ...
  describe('chain: lengthNotEquals', () => {
    ...
    it('fails when length matches', () => {
      ...
      expect(isArray<number>().lengthNotEquals(2).run([1, 2]).passes).toBe(
        false,
      );
    });
  });
  ...
  describe('chain: includes', () => {
    ...
    it('fails when item is not in array', () => {
      ...
      expect(isArray<string>().includes('c').run(['a', 'b']).passes).toBe(
        false,
      );
    });
  });
  ...
  describe('chain: inside', () => {
    it('passes when all items are in container', () => {
      expect(isArray<number>().inside([1, 2, 3]).run([1, 2]).passes).toBe(true);
      expect(isArray<number>().inside([5]).run([]).passes).toBe(true);
    });

    it('fails when some items are not in container', () => {
      expect(isArray<number>().inside([1, 2]).run([1, 2, 3]).passes).toBe(
        false,
      );
      expect(isArray<number>().inside([1]).run([2]).passes).toBe(false);
    });
  });

  describe('chain: notInside', () => {
    it('passes when some items are not in container', () => {
      expect(isArray<number>().notInside([3, 4]).run([1, 2]).passes).toBe(true);
      expect(isArray<number>().notInside([1]).run([2, 3]).passes).toBe(true);
    });

    it('fails when all items are in container', () => {
      expect(isArray<number>().notInside([1, 2, 3]).run([1, 2]).passes).toBe(
        false,
      );
    });
  });
  ...
  describe('chain: equals', () => {
    it('passes when arrays are the same reference', () => {
      const arr = [1, 2, 3];
      expect(isArray<number>().equals(arr).run(arr).passes).toBe(true);
    });

    it('fails when arrays have same content but different references', () => {
      expect(isArray<number>().equals([1, 2]).run([1, 2]).passes).toBe(false);
    });
  });
