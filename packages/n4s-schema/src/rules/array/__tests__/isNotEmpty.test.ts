...
describe('isNotEmpty', () => {
  it('passes for non-empty arrays', () => {
    ...
    expect(
      enforceLazy.isArray<string>().isNotEmpty().run(['a', 'b']).passes,
    ).toBe(true);
    expect(enforceLazy.isArray<null>().isNotEmpty().run([null]).passes).toBe(
      true,
    );
  });
  ...
  it('fails for empty arrays', () => {
    ...
    expect(enforceLazy.isArray<string>().isNotEmpty().run([]).passes).toBe(
      false,
    );
