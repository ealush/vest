...
describe('minLength', () => {
  it('passes when length meets minimum', () => {
    ...
    expect(enforceLazy.isArray<number>().minLength(1).run([1, 2]).passes).toBe(
      true,
    );
  ...
  it('fails when length is below minimum', () => {
    ...
    expect(enforceLazy.isArray<number>().minLength(3).run([1, 2]).passes).toBe(
      false,
    );
    expect(
      enforceLazy.isArray<string>().minLength(5).run(['a', 'b']).passes,
    ).toBe(false);
  });
});
