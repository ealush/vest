...
describe('lessThanOrEquals', () => {
  it('pass when number is less or equal', () => {
    ...
    expect(enforceLazy.isNumber().lessThanOrEquals(10).run(5).pass).toBe(true);
  });

  it('fails when number is greater', () => {
    expect(enforceLazy.isNumber().lessThanOrEquals(5).run(6).pass).toBe(false);
    expect(enforceLazy.isNumber().lessThanOrEquals(0).run(1).pass).toBe(false);
    expect(enforceLazy.isNumber().lessThanOrEquals(5).run(10).pass).toBe(false);
  });
});
