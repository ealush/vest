...
describe('checkKey', () => {
  describe('isKeyOf', () => {
    it('passes when key exists in object', () => {
      ...
      const keyB: any = 'b';
      const keyC: any = 'c';
      expect(enforceLazy.checkKey().isKeyOf(obj).run(keyA).passes).toBe(true);
      expect(enforceLazy.checkKey().isKeyOf(obj).run(keyB).passes).toBe(true);
      expect(enforceLazy.checkKey().isKeyOf(obj).run(keyC).passes).toBe(true);
    });
    ...
    it('passes when key exists with falsy value', () => {
      ...
      expect(enforceLazy.checkKey().isKeyOf(obj).run(keyB).passes).toBe(true);
      expect(enforceLazy.checkKey().isKeyOf(obj).run(keyC).passes).toBe(true);
      expect(enforceLazy.checkKey().isKeyOf(obj).run(keyD).passes).toBe(true);
    });
    ...
    it('passes for symbol keys', () => {
      ...
      const obj = { [sym]: 'value' };
      const key: any = sym;
      expect(enforceLazy.checkKey().isKeyOf(obj).run(key).passes).toBe(true);
    });
    ...
    it('fails when key does not exist', () => {
      ...
      const keyC: any = 'c';
      const keyX: any = 'x';
      expect(enforceLazy.checkKey().isKeyOf(obj).run(keyC).passes).toBe(false);
      expect(enforceLazy.checkKey().isKeyOf(obj).run(keyX).passes).toBe(false);
    });
    ...
    it('works with empty objects', () => {
      const obj = {};
      const key: any = 'a';
      expect(enforceLazy.checkKey().isKeyOf(obj).run(key).passes).toBe(false);
    });
    ...
    it('works with arrays', () => {
      ...
      const key2: any = 2;
      const key3: any = 3;
      expect(enforceLazy.checkKey().isKeyOf(arr).run(key0).passes).toBe(true);
      expect(enforceLazy.checkKey().isKeyOf(arr).run(key2).passes).toBe(true);
      expect(enforceLazy.checkKey().isKeyOf(arr).run(key3).passes).toBe(false);
    });
  });
  ...
  describe('isNotKeyOf', () => {
    ...
    it('works with empty objects', () => {
      const obj = {};
      const key: any = 'a';
      expect(enforceLazy.checkKey().isNotKeyOf(obj).run(key).passes).toBe(true);
    });
