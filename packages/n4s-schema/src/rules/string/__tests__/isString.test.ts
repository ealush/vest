...
describe('isString', () => {
  ...
  describe('startsWith', () => {
    it('passes when string starts with prefix', () => {
      expect(enforceLazy.isString().startsWith('he').run('hello').passes).toBe(
        true,
      );
      expect(enforceLazy.isString().startsWith('').run('hello').passes).toBe(
        true,
      );
      expect(enforceLazy.isString().startsWith('hel').run('hello').passes).toBe(
        true,
      );
    });

    it('fails when string does not start with prefix', () => {
      expect(enforceLazy.isString().startsWith('x').run('hello').passes).toBe(
        false,
      );
      expect(enforceLazy.isString().startsWith('lo').run('hello').passes).toBe(
        false,
      );
    });
  });
  ...
  describe('doesNotStartWith', () => {
    it('passes when string does not start with prefix', () => {
      expect(
        enforceLazy.isString().doesNotStartWith('x').run('hello').passes,
      ).toBe(true);
      expect(
        enforceLazy.isString().doesNotStartWith('lo').run('hello').passes,
      ).toBe(true);
    });

    it('fails when string starts with prefix', () => {
      expect(
        enforceLazy.isString().doesNotStartWith('he').run('hello').passes,
      ).toBe(false);
      expect(
        enforceLazy.isString().doesNotStartWith('hel').run('hello').passes,
      ).toBe(false);
    });
  });
  ...
  describe('endsWith', () => {
    it('passes when string ends with suffix', () => {
      ...
      expect(enforceLazy.isString().endsWith('').run('hello').passes).toBe(
        true,
      );
  ...
  describe('doesNotEndWith', () => {
    it('passes when string does not end with suffix', () => {
      expect(
        enforceLazy.isString().doesNotEndWith('x').run('hello').passes,
      ).toBe(true);
      expect(
  ...
  describe('matches', () => {
    it('passes when string matches regex', () => {
      expect(enforceLazy.isString().matches(/^h/).run('hello').passes).toBe(
        true,
      );
      expect(enforceLazy.isString().matches(/o$/).run('hello').passes).toBe(
        true,
      );
  ...
  describe('isNotBlank', () => {
    it('passes for strings with content', () => {
      ...
      expect(enforceLazy.isString().isNotBlank().run('hello').passes).toBe(
        true,
      );
      expect(enforceLazy.isString().isNotBlank().run(' x ').passes).toBe(true);
    });
  ...
  describe('minLength', () => {
    it('passes when string length is greater than or equal to minimum', () => {
      ...
      expect(enforceLazy.isString().minLength(2).run('hello').passes).toBe(
        true,
      );
  ...
  describe('maxLength', () => {
    ...
    it('fails when string length is greater than maximum', () => {
      ...
      expect(enforceLazy.isString().maxLength(2).run('hello').passes).toBe(
        false,
      );
    });
  });
  ...
  describe('lengthEquals', () => {
    it('passes when string length equals the specified value', () => {
      ...
      expect(enforceLazy.isString().lengthEquals(3).run('abc').passes).toBe(
        true,
      );
    });
  ...
  describe('lengthNotEquals', () => {
    it('passes when string length does not equal the specified value', () => {
      expect(
        enforceLazy.isString().lengthNotEquals(3).run('hello').passes,
      ).toBe(true);
      expect(enforceLazy.isString().lengthNotEquals(1).run('').passes).toBe(
        true,
      );
    });
    ...
    it('fails when string length equals the specified value', () => {
      expect(
        enforceLazy.isString().lengthNotEquals(5).run('hello').passes,
      ).toBe(false);
