...
describe('shorterThan', () => {
  it('passes when string length is less than specified value', () => {
    expect(enforceLazy.isString().shorterThan(6).run('hello').passes).toBe(
      true,
    );
  ...
  it('fails when string length is not less', () => {
    expect(enforceLazy.isString().shorterThan(5).run('hello').passes).toBe(
      false,
    );
    expect(enforceLazy.isString().shorterThan(3).run('hello').passes).toBe(
      false,
    );
