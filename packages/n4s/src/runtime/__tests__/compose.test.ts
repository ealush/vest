...
describe('enforce.compose', () => {
  it('Should create "and" relationship between composed rules', () => {
    const NumberAboveTen = enforce.compose(
      enforce.isNumber(),
  ...
  it('Should allow lazy evaluation of composed rules', () => {
    const NumericStringBetweenThreeAndFive = enforce.compose(
      enforce.isNumeric(),
      enforce.isString(),
  ...
  it('Should allow composing compositions', () => {
    ...
    const Entity = enforce.compose(
      enforce.loose({
        id: enforce.isNumeric(),
      })
    );

    const User = enforce.compose(Name, Entity);
