...
describe('enforce..message()', () => {
  it('Should set the failure message in builtin rules', () => {
    expect(
      enforce.message('oof. Expected true to be false').equals(false).run(true)
  ...
  it('Should set the failure message in custom rules', () => {
    expect(
      enforce.message('oof. Failed again!').ruleWithFailureMessage().run(true)
  ...
  describe('.message callback', () => {
    it('Should be passed the rule value as the first argument', () => {
      ...
      const arg = {};
      expect(enforce.message(msg).equals(false).run(arg)).toEqual(
    ...
    it('Should pass original messages the second argument if exists', () => {
      ...
      const arg = {};
      expect(
        enforce.message(msg).ruleWithFailureMessage(false).run(arg)
