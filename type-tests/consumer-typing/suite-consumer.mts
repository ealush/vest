...
type AtomicDraftsStayUsable = AssertTrue<
  IsEqual<
    DeepDraft<{
      buffer: ArrayBuffer;
      bytes: Uint8Array;
...
const fullUserResult = userSuite.run({ username: 'ann', age: 30 });
if (fullUserResult.valid) {
  expectTypeOf(fullUserResult.value).toEqualTypeOf<UserData>();
}
...
const focusedCoinRes = coinSuite.changed('amount').run({
  label: 'bus token',
  amount: '3.50',
});
if (focusedCoinRes.valid === true) {
  expectTypeOf(focusedCoinRes.value).toEqualTypeOf<DeepDraft<CoinOutput>>();
  // @ts-expect-error - DELIBERATE MISUSE: a focused valid result is not a complete output witness
  focusedCoinRes.value.amount.toFixed();
  if (typeof focusedCoinRes.value.amount === 'number') {
    focusedCoinRes.value.amount.toFixed();
  }
}

type FocusedCoinResult = FocusedSuiteResult<
  keyof CoinOutput,
  string,
  typeof coinSchema
>;
