...
function doesPackageExist(packageName: string): boolean {
  return packageNames.names[packageName] !== undefined;
}

function scaffold(config: InitAnswers): void {
...
function writeEntryPoint(packagePath: string, packageName: string): void {
...
function updateValues(packagePath: string, config: InitAnswers): void {
    ...
    if (fsExtra.lstatSync(file).isFile()) {
      ...
      Object.keys(configMapping).forEach(key => {
        content = content.replace(
          new RegExp(`{{${key}}}`, 'g'),
          config[configMapping[key]]?.toString() ??
            configMapping[key].toString(),
        );
      });
...
function removeTemplateExtensionFromFile(
  packagePath: string,
  _packageName: string,
): void {
...
const configMapping: Record<string, keyof InitAnswers | number | string> = {
