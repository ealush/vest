...
const schema = enforce.shape({
  startDate: enforce.isString(),
  endDate: enforce.isString(),
  timezone: enforce.isString(),
  total: enforce
    .isNumber()
    .dependsOn($ => [
      $.quantity,
...
A future interaction API can use the relationship graph:
...
| `include().when()` | records | `include` stays legacy escape hatch (unchanged) |
